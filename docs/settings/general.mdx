...
Specify where Vibe Kanban stores workspace data. Workspaces are created directly within this path.

- **Default location** - Leave empty to use the system default (`~/code/worktrees`)
- **Custom location** - Set a specific path if you prefer workspaces on a different drive or directory

<Warning>
Changes to Workspace Directory require an app restart. Existing workspaces may be migrated to the new root when they are reopened.
</Warning>
