...
- 2026-06-26 multi-line rich clipboard paste hotfix is live without backend restart:
  ...
  - chat paste now inserts multiline `text/plain` with `insertRawText` even when the clipboard also includes `text/html`
  - single-line rich HTML paste still uses the default Lexical path
  - release was created by copying current live `20260618Tbranch-search` and patching only the compiled paste handler; no VK restart was performed
...
- 2026-06-21 project sidebar flyout/customization is prepared in source but not deployed:
  ...
  - project edit dialog supports name, abbreviation, and pastel color selection
  - local project rename is backed by the existing project update API; archive updates remain supported
  - project navigation display overrides are stored in UI preferences scratch as `local_project_customizations`
  ...
  - no VK restart, frontend symlink switch, or live deploy was performed; frontend release plus backend build/restart are required for full live behavior
  - next deploy/restart package must rebuild forward from at least live `20260618Tbranch-search` and must not roll back the current frontend hotfix set
- 2026-06-18 branch base search hotfix is live without backend restart:
  - live frontend pointer is `/home/mcp/.local/share/vibe-kanban/frontend-dist/current -> /home/mcp/.local/share/vibe-kanban/frontend-dist/releases/20260618Tbranch-search`
  - live frontend asset is `/assets/index-CCBiSGB0.js`, sha256 `47a882a77d1d918006b7536e449927a72292b37fbc3299d513c43ff918bcf0dd`
  - base-branch selectors now tolerate separator/token search, so partial text like `program gen` can match `codex/program-generation-v3-llm-first`
  - the release carries forward the live markers for mobile archived projects, executor mode persistence, multi-line paste, code copy, rename, and queued UI behavior
  - live `python3 scripts/vk_live_regression_smoke.py` passed; backend PID remained `3435842`; no VK restart was performed
  - next restart package must preserve or rebuild forward from this frontend release; do not roll `frontend-dist/current` back to older staged assets
- 2026-06-12 refreshed restart package is staged but not restarted:
  - clean candidate worktree is `/home/mcp/vk-restart-candidate-20260611T112143Z`, branch `deploy/restart-candidate-20260611T112143Z`, commit `a055ce585b7b682dba53b5c99860f023f32e3ed3`
  - staged restart package is `/home/mcp/vk-restart-staging-20260612TissueReviewFlag`
  - next-restart backend binaries are installed at `/home/mcp/.local/bin/vibe-kanban-serve` and `/home/mcp/.local/bin/vibe-kanban-serve-prod`, both sha256 `32cebd5835faeae2007905e5b15593097699d735e8b7d0ea93020a4375238ed9`
  - running VK process is still PID `3435842`; no service restart was performed
  - staged frontend release is `/home/mcp/.local/share/vibe-kanban/frontend-dist/releases/20260612TissueReviewFlag`, asset `/assets/index-DA7R5Mdm.js`, sha256 `aa7021c20affeaae6eea6fc6715bf66d45da0a9d21fa58e8ba8cdf4ddaccf262`
  - live frontend pointer has since advanced to `/home/mcp/.local/share/vibe-kanban/frontend-dist/releases/20260618Tbranch-search`; do not activate the older staged frontend asset without rebuilding it forward
  - staged marker checks confirmed issue review flag strings plus queued capacity and active execution guard markers
  - restart is currently blocked by active workspace `FR::ORC::Generative Programming` on branch `codex/program-generation-v3-llm-first`
