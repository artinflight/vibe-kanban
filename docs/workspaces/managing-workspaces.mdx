...
<Info>
**Archiving preserves workspace data and Git work.** Vibe Kanban keeps your
conversation history, sessions, notes, branch, and commits. It may remove the
worktree copy after archive scripts finish, but only when no process uses the
workspace and the worktree has no uncommitted or untracked files. Unarchiving
recreates a removed worktree from its branch.
</Info>

<Warning>
If an archived worktree contains uncommitted or untracked files, Vibe Kanban
preserves it for manual review. Commit or otherwise secure the files before you
expect the worktree to be reclaimed.
</Warning>
...
|--------|----------|--------------|--------------|
| **Archive** | Removed when inactive and Git-clean | Preserved | Yes - unarchive anytime |
...
Worktrees are stored in `~/code/worktrees` by default:

| Platform | Default Location |
|----------|-----------------|
| **macOS** | `~/code/worktrees` |
| **Linux** | `~/code/worktrees` |
| **Windows** | `~/code/worktrees` |

You can configure a custom location in **Settings → General → Workspace Directory**. When set, worktrees are stored directly in `{your-path}`.
...
<Tab title="Windows">
```powershell
Get-ChildItem "$HOME\code\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
...
2. **Review deferred archives** - Pinned, running, process-referenced, or Git-dirty workspaces remain on disk
3. **Configure cleanup scripts** - Add cleanup scripts to remove build artifacts (like `node_modules`) when workspaces close

<Tip>
Large generated directories can consume substantial space in each worktree.
Configure an archive script only for exact paths your project has verified as
reproducible. A directory name such as `target`, `build`, `cache`, or
`node_modules` does not prove that its contents are disposable.
</Tip>
